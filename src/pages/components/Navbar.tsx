        ...
        <div className="flex justify-between items-center py-4">
          <div className="flex w-full flex-row justify-between items-center">
            <Link to="/" className="flex flex-row gap-2 items-center">
              <img src={yomlogo} alt="" className="w-10 md:w-20 h-10 md:h-20" />
          ...
          {/* Mobile Nav */}
          <div
            className={`md:hidden bg-[#0E0D17] text-[#fff] fixed z-50 w-full h-1/2 top-0 overflow-y-auto bottom-0 pt-12 pl-4
          duration-500 ${isOpen ? "top-0" : "top-[-100%]"}`}
          >
            <div className="cursor-pointer absolute right-10 flex items-center">
            ...
            <div className="flex flex-col justify-center items-center pt-12 gap-8 mx-auto">
              {NavLinks.map((item) => (
                <div key={item.id}>
                  <Link to={item.link} className="font-[600]">
                    {item.text}
                  </Link>
                </div>
              ))}
