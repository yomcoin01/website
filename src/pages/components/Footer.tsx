  ...
  return (
    <div>
      <div className="container">
        <div className="grid grid-cols-1 md:grid-cols-2 justify-start items start gap-10 py-12">
          <div className="flex flex-col gap-4 md:pr-12 pr-0">
            <div className="flex flex-row gap-2 items-center">
              <img src={yomlogo} alt="" style={{ maxWidth: "50%" }} />
              {/*<p className="font-[800] text-2xl">Yomcoin</p>*/}
            </div>
            <p className="text-txtCol text-sm font-[600]">
              Yomcoin is a leading platform in the decentralized finance (DeFi)
              space, committed to driving innovation and positive change in the
              financial industry. Join us on our journey to transform the future
          ...
          <div className="flex flex-col md:flex-row gap-4 md:gap-0 justify-between items-start">
            <div>
              <p className="font-[600] text-xl pb-0 md:pb-4">Company</p>
              <ul>
                {complinks.map((item, index) => (
                  <li
                    key={index}
                    className="text-txtCol text-md font-[600] cursor-pointer"
                  >
                    {item}
                  </li>
                ))}
            ...
            <div>
              <p className="font-[600] text-xl pb-0 md:pb-4">Resources</p>
              <div>
            ...
            <div>
              <p className="font-[600] text-xl pb-0 md:pb-4">Partnership</p>
              <ul>
                {partlinks.map((item, index) => (
                  <div key={index}>
                    <Link
                      to={item.link}
                      className="text-txtCol text-md font-[500]"
                    >
                      {item.text}
                    </Link>
                  </div>
        ...
        <hr />
        <p className="text-center md:py-4 py-2">
          © 2024 Yomcoin. All rights reserved.{" "}
        </p>
      </div>
    </div>
  );
